/**
 * ============================================================
 * KatalogView — Halaman Katalog Produk Bawang Goreng
 * ============================================================
 * Fitur baru yang diimplementasikan:
 *  - SEARCH (debounce 350ms): pencarian global dari Header otomatis
 *    diterapkan setelah user berhenti mengetik (lihat useProductTable)
 *  - FILTER kategori produk (Semua / Original / Pedas / Kiloan)
 *  - SORTING (Paling Relevan / Best Seller / Harga / Nama A-Z)
 *  - PAGINATION (6 kartu per halaman) via komponen <Pagination/>
 *  - RESET FILTER: kembalikan kategori, sorting, dan pencarian
 *  - LOADING SKELETON saat pertama kali data diambil dari API
 *
 * Semua state fitur di atas dikelola satu hook: useProductTable
...
// Definisi pill filter kategori (key 'all' = semua kategori)
const CATEGORY_FILTERS = [
  { key: 'all', label: 'Semua' },
  { key: 'Original', label: 'Original' },
  { key: 'Pedas', label: 'Pedas' },
  { key: 'Kiloan', label: 'Kiloan' },
];

// Opsi dropdown "Urutkan" yang tersedia di halaman katalog
const SORT_OPTIONS = [
...
export function KatalogView({
  ...
  // externalSearch = pencarian global dari Header (di-debounce otomatis).
  // pageSize 6 = 6 kartu produk per halaman.
  const table = useProductTable(products, { pageSize: 6, externalSearch: searchQuery });

  // --- RESET FILTER ---
  // Bersihkan pencarian Header + kembalikan kategori/sorting/halaman
    ...
    <div className="flex flex-col w-full pb-16">
      ...
      <section id="katalog-grid" className="scroll-mt-24">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          {/* Dropdown kriteria sorting (relevan / best seller / harga / nama) */}
          <div className="flex items-center gap-2 text-xs text-[#554336] font-['Work_Sans'] shrink-0">
            ...
            <select
              ...
              className="bg-white border border-[#dbc2b0]/30 rounded-xl px-3 py-1.5 font-bold text-[#8d4b00] focus:outline-none focus:ring-2 focus:ring-[#8d4b00]/20 cursor-pointer shadow-xs"
            >
          ...
          {/* Indikator "Memuat..." saat fetch API + tombol Reset Filter */}
          <div className="flex items-center gap-2">
            {table.loading && (
              <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-[#8d4b00] font-['Work_Sans']">
            ...
            <button
              onClick={handleReset}
              className="px-4 py-2 rounded-xl bg-[#fff1eb] text-[#554336] hover:bg-[#ffeae1] font-['Work_Sans'] text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-1.5 shadow-xs border border-[#dbc2b0]/20"
            >
        ...
        {table.loading && table.data.length === 0 ? (
          /* --- LOADING SKELETON --- */
          /* Kartu-kartu placeholder (animate-pulse) saat data pertama
             kali diambil dari API agar layout tidak melompat. */
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="bg-white rounded-3xl overflow-hidden shadow-xs border border-[#dbc2b0]/30 animate-pulse flex flex-col"
              >
                <div className="h-60 sm:h-64 bg-[#ffeae1]"></div>
                <div className="p-6 space-y-3">
                  <div className="h-4 w-2/3 bg-[#ffeae1] rounded"></div>
                  <div className="h-3 w-1/3 bg-[#ffeae1] rounded"></div>
                  <div className="h-3 w-full bg-[#ffeae1] rounded"></div>
                  <div className="h-10 w-full bg-[#ffeae1] rounded-xl"></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <>
            {/* --- GRID PRODUK (data ter-filter, ter-sort, halaman aktif) --- */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {table.data.map((p) => (
                <div
                  key={p.id}
            ...
            {/* --- PAGINATION (6 kartu per halaman) --- */}
            <div className="mt-6 rounded-2xl overflow-hidden border border-[#dbc2b0]/30 shadow-xs">
              <Pagination
